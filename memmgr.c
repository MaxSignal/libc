...
void set_mem_pool()
{
    ...
    if (hdr != 0) {
        if (hdr->model[0] == 'C') {
            ...
            if (model < 160) {
                pool_size = 3072 * 1024;
                pool = (byte *)0x8c4be400;
            } else {
                pool_size = 8192 * 1024;
                pool = (byte *)0x8c800000;
            }
        }
    }
}
...
void memmgr_print_stats()
{
    ...
    log_write("------ Memory manager stats ------\n\n");
    log_write(    "Pool: free_pos = %u (%u bytes left)\n\n\n", 
    ...
    {
        log_write(    "  * Addr: 0x%08x; Size: %08u\n",
                p, p->s.size);
    ...
    if (freep)
    {
        ...
        while (1)
        {
            log_write(    "  * Addr: 0x%08x; Size: %08u; Next: 0x%08x\n", 
                    p, p->s.size, p->s.next);
...
void *memmgr_realloc(void *ptr, ulong size)
{
	...
	if (!size) {
		memmgr_free(ptr);
		return memmgr_alloc(0);
	}

	newptr = memmgr_alloc(size);
	if (newptr) {
		block = ((mem_header_t*) ptr) - 1;
		ulong old_size = (block->s.size - 1) * sizeof(mem_header_t);
